...
void apply_bdt(const char* TupleName, int year, const char* mag="Combined", const char* charge="Combined") {

    // This script applies the 'full' BDT
    // to either MC or data according to
    // 'sim' flag
    
    // It takes arguments such that it can
    // be easily interfaced with a batch
    // submission script
    
    //=== Get tree ===//
    //TFile* g = TFile::Open(Form("/data/lhcb/users/colmmurphy/ReducedTrees/%i/Mag%s/%sCharge/Full_%s.root", year, mag, charge, TupleName), "READ");
    TFile* g = TFile::Open("/data/lhcb/users/colmmurphy/gangadir/workspace/colmmurphy/LocalXML/351/Data_Bu2DH_AllModes_2011_MagUp.root");
    TTree* dataTree = (TTree*)g->Get("B2DPi_D2KPiPi0/DecayTree");
    
    //=== Create output file ===//
    TFile* f = new TFile(Form("/data/lhcb/users/colmmurphy/ReducedTrees/%i/Mag%s/%sCharge/FullBDT_%s_new.root", year, mag, charge, TupleName), "RECREATE");
    TTree* tree   = dataTree->CloneTree(0);
    tree->SetMaxTreeSize(400000000000LL);
    ...
    reader->AddVariable("Pi0_PT", &var[5]);
    reader->AddVariable("Pi0_P", &var[6]);
    //=== Neutral particle variables ===//
    reader->AddVariable("Gamma1_CL", &var[7]);
    reader->AddVariable("Gamma2_CL", &var[8]);
    //=== Vertex Chi^2 ===//
    reader->AddVariable("Bu_VTXCHI2DOF", &var[9]);
    //reader->AddVariable("D0_VTXCHI2DOF", &var[13]);
    //=== Impact parameters ===//
    //reader->AddVariable("Bu_MIPCHI2_PV", &var[14]);
    reader->AddVariable("log(D0_MIPCHI2_PV) := log(D0_MIPCHI2_PV)", &var[10]);
    reader->AddVariable("log(Bach_MIPCHI2_PV) := log(Bach_MIPCHI2_PV)", &var[11]);
    //reader->AddVariable("log(K0_MIPCHI2_PV) := log(K0_MIPCHI2_PV)", &var[12]);
    //reader->AddVariable("log(P0_MIPCHI2_PV) := log(P0_MIPCHI2_PV)", &var[18]);
    //=== Flight distance ===//
    reader->AddVariable("log(Bu_FDCHI2_OWNPV) := log(Bu_FDCHI2_OWNPV)", &var[13]);
    reader->AddVariable("log(D0_FDCHI2_OWNPV) := log(D0_FDCHI2_OWNPV)", &var[14]);
    //=== Director angle (DIRA) ===//
    reader->AddVariable("log(1-Bu_DIRA_BPV) := log(1-Bu_DIRA_BPV)", &var[15]);
    reader->AddVariable("log(1-D0_DIRA_BPV) := log(1-D0_DIRA_BPV)", &var[16]);
    ...
    //=== P_T asymmetry (ptasy) ===//
    reader->AddVariable("Bu_ptasy_1.50", &var[18]);  
    
    
    //===============================================================//
    //=== Add the ML variables one wants calculated to the reader ===//
    //===============================================================//
    reader->BookMVA("BDT method", "/home/colmmurphy/dataset/weights/jicAllMC_B2DPi_D2KPiPi0_AllDataYears_MagCombined_CombinedCharge_BDT.weights.xml");
    ...
    for (Int_t i=0; i<dataTree->GetEntries(); i++) {
     ...
     /*
        ...
        */
        ...
        var[5]  = (Float_t)  uservar["Pi0_PT"];
        var[6]  = (Float_t)  uservar["Pi0_P"];
        var[7] = (Float_t)  uservar["Gamma1_CL"];
        var[8] = (Float_t)  uservar["Gamma2_CL"];
        var[9] = (Float_t)  uservar["Bu_VTXCHI2DOF"];
        //var[13] = (Float_t)  uservar["D0_VTXCHI2DOF"];
        //var[14] = (Float_t)  uservar["Bu_MIPCHI2_PV"];
        var[10] = (Float_t)  TMath::Log(uservar["D0_MIPCHI2_PV"]);
        var[11] = (Float_t)  TMath::Log(uservar["Bach_MIPCHI2_PV"]);
        //var[12] = (Float_t)  TMath::Log(uservar["K0_MIPCHI2_PV"]);
        //var[18] = (Float_t)  TMath::Log(uservar["P0_MIPCHI2_PV"]);
        var[13] = (Float_t)  TMath::Log(uservar["Bu_FDCHI2_OWNPV"]);
        var[14] = (Float_t)  TMath::Log(uservar["D0_FDCHI2_OWNPV"]);
        var[15] = (Float_t)  TMath::Log(1-uservar["Bu_DIRA_BPV"]);
        var[16] = (Float_t)  TMath::Log(1-uservar["D0_DIRA_BPV"]);
        ...
        var[18] = (Float_t)  uservar["Bu_ptasy_1.50"];
        
       
        //=== Calculate the BDT value using the TMVA::Reader ===//
        FullBDT = reader->EvaluateMVA("BDT method");
        if (FullBDT == -999) continue;
